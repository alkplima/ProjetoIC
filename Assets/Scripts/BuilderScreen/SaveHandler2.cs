...
public class SaveHandler2 : Singleton<SaveHandler> {
                    ...
                    if (guidToTileBase.ContainsKey(tile.guidForBuildable)) {
                        ...
                        switch (guidToTileBase[tile.guidForBuildable].ToString())
                        {
                            case "Moeda (UnityEngine.Tilemaps.AnimatedTile)":
                                Instantiate(moeda, tile.position, Quaternion.identity);
                                break;
                                //adicionar/instanciar aqui os outros elementos do jogo
